...
public class LegitMiddleClick extends Module {
    ...
    private final Setting<Integer> swapBackDelay = sgGeneral.add(new IntSetting.Builder()
        ...
        .description("Delay in milliseconds before swapping back to the original item.")
        .defaultValue(0)
        .min(0)
        .sliderMax(1000)
        .visible(() -> mode.get() != Mode.AddFriend && !quickSwap.get() && swapBack.get())
        .build()
    );
    ...
    public LegitMiddleClick() {
        super(AddonTemplate.CATEGORY, "legit-middle-click", "Perform various actions when you middle click.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        // Check if it's time to swap back (independent of isUsing)
        if (swapBackTime > 0 && System.currentTimeMillis() >= swapBackTime) {
            swapBackTime = 0;
            InvUtils.swapBack();
        }
    ...
    private void stopIfUsing(boolean wasCancelled) {
        if (isUsing) {
            scheduleSwapBack(wasCancelled);
            mc.options.useKey.setPressed(false);
            isUsing = false;
        }
    }

    private void scheduleSwapBack(boolean wasCancelled) {
        if (wasHeld) return;
        ...
        if (quickSwap.get()) {
            ...
            int delay = swapBackDelay.get();
            if (delay <= 0) {
                // No delay, swap immediately
                InvUtils.swapBack();
            } else {
                // Schedule swap back after delay
                swapBackTime = System.currentTimeMillis() + delay;
            }
        }
    }
